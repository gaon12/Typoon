...
object LanguageScorer {
    fun englishTokenScore(token: String): Float {
        ...
        val isKnownEnglishWord =
            lower in ConversionConstants.COMMON_ENG_WORDS || looksLikeUrlOrEmail(token)
        return when {
            token.isEmpty() -> 0f
            isKnownEnglishWord -> 1.0f
            englishLetterCount == 0 -> 0f
            else -> {
    ...
    fun koreanTokenScore(token: String): Float {
        ...
        return when {
            token.isEmpty() -> 0f
            token in ConversionConstants.COMMON_KOR_WORDS -> 1.0f
            koreanCount == 0 -> 0f
            else -> {
